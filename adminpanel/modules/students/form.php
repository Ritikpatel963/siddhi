<div class="panel col-xl-7">
    <form method="post">
        ...
        <div class="row g-3">
            ...
            <div class="col-12"><label class="form-label">Enrolled courses</label><select class="form-select" name="course_ids[]" multiple size="<?= max(3, min(6, count($courses))) ?>"><?php foreach ($courses as $course): ?><option value="<?= e($course['id']) ?>" <?= in_array((int) $course['id'], $enrolledCourseIds, true) ? 'selected' : '' ?>><?= e($course['name']) ?></option><?php endforeach; ?></select><div class="form-text">Hold Ctrl (Windows) or Command (Mac) to select more than one.</div></div>
            <div class="col-md-6">
